...
class ModbusRS485SnifferGUI:
    def __init__(self, master):
        ...
        self.start_button.grid(row=0, column=6, padx=(0, 2))
        self.stop_button = ttk.Button(frame, text="Стоп", command=self.stop_sniffer, state="disabled")
        self.stop_button.grid(row=0, column=7, padx=(0, 2))
    ...
    def log_to_file(self, msg):
        ...
        with open("modbus_sniffer_log.txt", "a") as log_file:
                        ...
                        while idx + 4 <= len(buffer):  # Минимум 4 байта на кадр
                            for end in range(idx+4, len(buffer)+1):
                                ...
                                if crc_calc == crc_recv:
                                    ...
                                    address = candidate[0]
                                    if self.addr_filter_var.get() and str(address) != self.addr_filter_var.get():
                                        idx = end
                                        continue
